...
describe("Batch Loading and Related Data", () => {
  ...
  test("should not reload objects already in context", async () => {
    ...
    const firstLoadResults = await Promise.all(
      testPostsSlice.map((post) =>
    ...
    const secondLoadResults = await Promise.all(
      testPostsSlice.map((post) =>
        post.loadRelatedData(["userId"], loaderContext),
      ),
    );
  ...
  test("should batch requests within batchDelay window", async () => {
    ...
    const loadPromises = testPostsSlice.map(
      (post, i) =>
        new Promise((resolve) =>
          setTimeout(
            () => resolve(post.loadRelatedData(["userId"], loaderContext)),
            i * 2,
          ),
        ),
    );
  ...
  test("should handle duplicate requests within same batch", async () => {
    ...
    const duplicatePromises = Array(10)
      .fill()
      .map(() => User.find(user.userId, { batchDelay: 50, loaderContext }));
    ...
    const totalCapacity = results.reduce(
      (sum, result) => sum + result.getNumericConsumedCapacity("read", true),
      0,
    );
