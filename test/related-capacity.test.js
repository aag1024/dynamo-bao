  ...
  describe("Update Operations", () => {
    test("should track capacity for update operations", async () => {
      await runWithBatchContext(async () => {
        const result = await verifyCapacityUsage(
          async () =>
            await Comment.update(testComment.getPrimaryId(), {
  ...
  describe("Edge Cases", () => {
    ...
    test("should track capacity when loading related data returns null", async () => {
      await runWithBatchContext(async () => {
        ...
        expect(orphanComment.getNumericConsumedCapacity("write", true)).toBe(
          2.0,
        );
        expect(orphanComment.getNumericConsumedCapacity("read", true)).toBe(
          0.5,
        );
      });
    });
    ...
    test("should accumulate capacity for repeated related data loads", async () => {
      await runWithBatchContext(async () => {
        ...
        const firstLoadCapacity = comment.getNumericConsumedCapacity(
          "read",
          true,
        );

        await comment.loadRelatedData(["postId"]);
        const secondLoadCapacity = comment.getNumericConsumedCapacity(
          "read",
          true,
        );
