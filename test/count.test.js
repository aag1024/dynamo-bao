...
describe('Count Query Tests', () => {
  ...
  test('should respect limit for both regular and count queries', async () => {
    // Test with countOnly
    const countResult = await TestUser.queryByIndex('byStatus', 'active', null, {
      countOnly: true,
      limit: 1
    });

    // Test without countOnly
    const regularResult = await TestUser.queryByIndex('byStatus', 'active', null, {
      limit: 1
    });
  ...
  test('should respect limit with countOnly queries', async () => {
    const limitedCount = await TestUser.queryByIndex('byStatus', 'active', null, {
      countOnly: true,
      limit: 1
    });
  ...
  test('should use default limit when no limit specified', async () => {
    const defaultResult = await TestUser.queryByIndex('byStatus', 'active', null, {
      countOnly: true
    });

    const explicitResult = await TestUser.queryByIndex('byStatus', 'active', null, {
      countOnly: true,
      limit: 1
    });
